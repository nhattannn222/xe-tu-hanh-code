...
# Các biến để theo dõi hướng
a = b = c = 1

while cap.isOpened():
    ret, img = cap.read()
    if not ret:
        break  # Nếu không đọc được khung hình, thoát vòng lặp
    ...
    l = r = 0
    if lines is not None:  # Kiểm tra xem có phát hiện đường thẳng không
        for line in lines:
            for x1, y1, x2, y2 in line:
                if (round(x2 - x1) != 0):
                    arctan = slope(x1, x2, y1, y2)

                    # Kiểm tra xem đường thẳng nằm trong vùng nhất định không
